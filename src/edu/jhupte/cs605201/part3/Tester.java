...
final public class Tester {
	...
	static public void main(String[] args) {
		...
		// Create a Room instance with the correct dimensions.
		// This assumes that Room has a length, with and
		// height parameterized constructor.
		Room room = new Room(17.0, 8.0, 8.5);
		// Create opening objects to insert into the walls.
		// There are only two different sizes, so one of each.
		// This assumes that Surface has a length, width
		// parameterized constructor.
		Rectangles window = new Surface().setWidth(4).setLength(3);
		Rectangles door = new Surface(6.75, 3.5);
		// Add the openings to the proper walls
		room.getNorthWall().addOpening(window);
		room.getEastWall().addOpening(window);
		room.getSouthWall().addOpening(window);
		room.getSouthWall().addOpening(window);
		room.getWestWall().addOpening(door);

		// ceiling and wall areas using models
		ceilingArea = room.getCeilingArea();
		wallArea = room.getWallArea();
